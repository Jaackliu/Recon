...
后端的 `parser.py` 在接收到 AI API 返回的中间 JSON 后，必须执行以下数据清洗与校验逻辑，才能最终写入 `transactions.json`。
...
*   **条件判定**：
    ...
    2.  在支出发生后的 **3天内**，账户 B (B != A) 出现一笔 `type_code: 1` (收入)。
    3.  金额匹配约束：`支出金额 * 97% <= 收入金额 <= 支出金额` (考虑潜在的手续费磨损)。
*   **执行动作**：
...
*   **异常处理 (transactions_check.py)**：若发现不一致，记录 PDF 文件名/哈希与出错交易明细，并将该 PDF 对应交易从 `transactions.json` 与 `parsed.json` 删除，便于重跑。
