...
*   `render_error`：PDF 渲染失败（文件损坏或格式不支持）
*   `ai_no_response`：AI API 未返回响应（网络错误或 API 限流）
*   `ai_parse_error`：AI 返回内容无法解析为 JSON
*   `no_valid_transactions`：AI 返回的交易数据缺少必要字段或格式无效
*   `multi_account`：单个 PDF 中检测到多个不同账户代码
*   `balance_check_failed`：余额一致性校验失败（经 3 次重试后仍不通过）
