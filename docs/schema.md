...
### 5. `parse_summary.json` (解析结果摘要)
由 `parser.py` 在每次解析完成后写入，供 `api_server.py` 读取并生成详细的用户通知。记录本次解析的成功/失败详情。

结构为 JSON 对象，包含以下字段：

| 字段名 | 键名 (Key) | 数据类型 | 说明 / 约束 |
| :--- | :--- | :--- | :--- |
| **处理时间** | `timestamp` | String | 本次解析的处理时间 (ISO 8601 格式)。 |
| **待处理 PDF 数** | `total_pdfs` | Integer | 本次扫描到的待处理 PDF 总数。 |
| **新 PDF 数** | `new_pdfs` | Integer | 本次实际尝试解析的 PDF 数量。 |
| **成功数** | `success_count` | Integer | 成功解析并入库的 PDF 数量。 |
| **新增交易数** | `new_transaction_count` | Integer | 本次新增的交易记录总数。 |
| **失败列表** | `failed_pdfs` | Array[Object] | 失败的 PDF 详情数组，每个对象包含 `file_name`、`reason`、`detail`。 |
| **成功列表** | `success_pdfs` | Array[Object] | 成功的 PDF 详情数组，每个对象包含 `file_name`、`transaction_count`。 |

**失败原因 (`reason`) 枚举：**

*   `render_error`：PDF 渲染失败（文件损坏或格式不支持）。**不重试**（文件本身损坏，重试无意义）。
*   `ai_no_response`：AI API 未返回响应（网络错误或 API 限流）。**不重试外层**（`call_ai_grouped` 内部已有 3 次重试）。
*   `ai_parse_error`：AI 返回内容无法解析为 JSON。**重试 3 次**，每次重新调用 AI。
*   `no_valid_transactions`：AI 返回的交易数据缺少必要字段或格式无效。**重试 3 次**，每次重新调用 AI。
*   `multi_account`：单个 PDF 中检测到多个不同账户代码。**重试 3 次**，每次重新调用 AI。
*   `balance_check_failed`：余额一致性校验失败。**重试 3 次**（在 `run_balance_check_and_reparse` 中，每次重新解析 PDF）。

### 6. 解析中止 API (Parse Abort)

`POST /<user_id>/api/parse/abort` 端点用于中止正在运行的解析进程。

*   **触发方式**：前端长按"解析 PDF"按钮（600ms），按钮逐渐变红，弹出确认窗口后用户确认中止。
*   **执行动作**：向 parser.py 子进程发送 `SIGTERM`，等待 5 秒后若未退出则强制 `SIGKILL`。
*   **状态清理**：重置 `_parse_status` 为 `{"running": False, "ok": False}`，移除进程引用。
*   **用户通知**：写入 `msg.parse_aborted` 通知消息（"PDF 解析已被用户手动中止"）。
*   **前端响应**：显示"解析已中止" Toast 提示，按钮恢复可点击状态。
...
*   **倒序 PDF 支持**：银行账单可能按时间倒序排列（日期从近到远，每天从晚到早）。parser 在分配 `transaction_id` 前，自动检测同一 `(account, date, currency)` 组内交易的余额顺序，若为倒序则自动反转为正序，确保序列号始终按时间正序分配。
